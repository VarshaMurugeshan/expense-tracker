...
mongoose.connect("mongodb+srv://varshamurugeshan:varshamr@cluster0.gb2dd.mongodb.net/expenses").then(()=>{
...
app.put("/api/expense/:id",async()=>{
   ...
   const {title,amount}=req.body
   try{
    const UpdatedItem=await Expense.findOneAndUpdate({id},
    {title,amount},{new:true});
    if(!updatededExpense){
        res.status(200).json({message:"Expense not found"});
        return
    }
